...
- **LLM control plane**: `.agentic-memory/`, which contains the vault version, concise agent instructions, and templates.
...
A coding harness that starts in this repo should automatically read the root `AGENTS.md`. That file is the entry point; it routes the agent to `.agentic-memory/LLMS.md`, which then routes to `MEMORY.md`, `USER.md`, optional instruction files, and only the relevant memory content. `BOOTSTRAP.md` is not needed for this mode.

### 2. Cross-project persistence

Use this when an agent is working in some other repo but should also preserve durable information in a central Agentic Memory vault.

Copy/adapt `BOOTSTRAP.md` into a harness entry point such as a user-level `AGENTS.md`, `CLAUDE.md`, Pi `APPEND_SYSTEM.md`, or a local project equivalent. The bootstrap points at the central vault by absolute path, keeps the current project task primary, and uses the vault as secondary memory only when it is useful.

If a global bootstrap is active while the harness is started inside the memory vault itself, the vault-local entry point wins. The bootstrap is redundant in that context and should not cause a second cross-project memory flow.
...
6. For cross-project use, copy/adapt `BOOTSTRAP.md` into the relevant global or project harness entry point and replace `/absolute/path/to/memory-vault` with the real path.
...
```text
BOOTSTRAP.md               # snippet humans adapt into harness-specific entry points
...
- [Bootstrap](docs/bootstrap.md) — how to connect a harness to a central Agentic Memory vault.
