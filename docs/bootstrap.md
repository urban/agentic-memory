...
`BOOTSTRAP.md` is the adapter snippet for the cross-project Agentic Memory entry point.
...
- **Cross-project persistence** starts when a harness is opened somewhere else and a global or project-level instruction file points at a central Agentic Memory vault by absolute path.

Use `BOOTSTRAP.md` only for the second case. It should point the harness to the central vault entry points instead of duplicating the vault's full `.agentic-memory/` control plane.

## When to use it

Install a bootstrap when an agent working in one repo should be able to read and update durable memory stored in a central vault.
...
You do not need `BOOTSTRAP.md` for agents already operating inside the memory vault. In that case, the vault's own `AGENTS.md` is the entry point.

## Global bootstrap behavior

A global bootstrap may be active in every repo, including the memory vault itself. To avoid confusion:
...
- if the current working directory is the central vault, follow the vault-local `AGENTS.md` path and treat the bootstrap as redundant
- do not create a separate project-memory flow merely because the global bootstrap is present

This makes a global user-level `AGENTS.md`, `CLAUDE.md`, or `APPEND_SYSTEM.md` safe to use alongside a vault-local setup.

## Local project bootstrap behavior

A repo-level `AGENTS.md` or `CLAUDE.md` can include the bootstrap when the project itself is not an Agentic Memory vault but should persist durable context to one.

In that setup, the project file should keep its normal project instructions and add the bootstrap as a secondary-memory adapter. The current repo's task remains first; memory updates happen at natural stopping points.

## Setup

1. Open repository-level `BOOTSTRAP.md`.
